...
import com.musicdistribution.streamingservice.application.service.AlbumService;
import com.musicdistribution.streamingservice.application.service.PaymentService;
...
public class AlbumResource {
     ...
     * @param token        - the payment provider token generated for the transaction which was made by the user.
    ...
    public ResponseEntity<AlbumResponse> publish(
        ...
        Boolean isValidTransaction = paymentService.capturePayment(token);
        return (isValidTransaction) ? this.albumService.publish(albumRequest, cover,
    ...
    public ResponseEntity<AlbumResponse> raiseTier(
        ...
        Boolean isValidTransaction = paymentService.capturePayment(token);
        return (isValidTransaction) ? this.albumService.raiseTier(albumRequest,
