        ...
        if (transactionType && transactionObject) {
            switch (transactionType) {
                case TRANSACTION_TYPE.SONG_PUBLISH:
                    ...
                    return SongRepository.publishSong(songCover, transactionObject);
                case TRANSACTION_TYPE.SONG_RAISE_TIER:
                    return SongRepository.raiseTierSong(JSON.parse(transactionObject));
                case TRANSACTION_TYPE.ALBUM_PUBLISH:
                    const albumCover = FileUtil.convertToBlob(localStorage.getItem(COVER));
                    return AlbumRepository.publishAlbum(albumCover, transactionObject);
                case TRANSACTION_TYPE.ALBUM_RAISE_TIER:
                    return AlbumRepository.raiseTierAlbum(JSON.parse(transactionObject));
                default:
                    return undefined;
            }
        }
    ...
    useEffect(() => {
        if (isCheckoutValid()) {
            getResult().then(result => {
                setMessage((result !== undefined) ? TRANSACTION_SUCCESSFUL : TRANSACTION_FAILURE);
                setDisplayCheckmark(true);
                setResultWatermark((result !== undefined));
            }).catch(() => {
