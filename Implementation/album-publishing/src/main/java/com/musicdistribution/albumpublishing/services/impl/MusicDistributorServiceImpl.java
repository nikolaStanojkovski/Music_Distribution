...
public class MusicDistributorServiceImpl implements MusicDistributorService {
    ...
    public Optional<PublishedAlbum> publishAlbum(PublishedAlbumRequest publishedAlbumRequest) {
        Objects.requireNonNull(publishedAlbumRequest, "Album to be published must not be null.");
        var constraintViolations = validator.validate(publishedAlbumRequest);
        if (constraintViolations.size() > 0) {
            throw new ConstraintViolationException("The published album form is not valid", constraintViolations);
        }
