                ...
                override fun onResponse(
                    ...
                    if (response?.body() == null) {
                        ...
                        if (listener != null) {
                            when (entityType) {
                                EntityType.ARTISTS -> {
                                    artistsLiveData.value =
                                        if (listener.favouriteArtists.isNullOrEmpty()) mutableListOf() else listener.favouriteArtists
                                }
                                EntityType.ALBUMS -> {
                                    albumsLiveData.value =
                                        if (listener.favouriteAlbums.isNullOrEmpty()) mutableListOf() else listener.favouriteAlbums
                                }
                                EntityType.SONGS -> {
                                    songsLiveData.value =
                                        if (listener.favouriteSongs.isNullOrEmpty()) mutableListOf() else listener.favouriteSongs
                                }
                            }
                        } else {
                            Toast.makeText(
